            ...
            try
            {
                ...
                if (Settings.GrenadeESP)
                {
                    ...
                    foreach (var throwable in _grenadeList)
                    {
                        if (throwable == null)
                            continue;
                        int distance = (int)Vector3.Distance(Main.LocalPlayer.Transform.position,
                        ...
                        Render.DrawString(new Vector2(screenPosition.x - 50f, screenPosition.y), text,
                            Settings.SpecialColor);

                        if (distance < 10f)
                        {
                            var textStyle = new GUIStyle(GUI.skin.label) { fontSize = 25 };
                            GUI.Label(new Rect(1024, Screen.height - 48, 512, 48), $"Grenade close: {distance}",
                                textStyle);
                        }
